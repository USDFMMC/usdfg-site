/**
 * TEMPORARY pre-launch arena gate UI — delete with src/temporary/arena-password-gate/.
 */
...
import {
  ARENA_ACCESS_PASSWORD,
  ARENA_ACCESS_STORAGE_KEY,
} from '@/temporary/arena-password-gate/constants';

interface ArenaPasswordFormProps {
  onSuccess: () => void;
}

export default function ArenaPasswordForm({ onSuccess }: ArenaPasswordFormProps) {
  const [password, setPassword] = useState('');
  const [error, setError] = useState<string | null>(null);

  const handleSubmit = () => {
    const trimmed = password.trim();
    if (!ARENA_ACCESS_PASSWORD) {
      setError('Arena gate is not configured (missing VITE_ARENA_ACCESS_PASSWORD).');
      return;
    }
    if (trimmed === ARENA_ACCESS_PASSWORD) {
      setError(null);
      try {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-void">
      ...
      >
        ...
        <p className="font-body text-white/70 text-center mb-6">
          The browser arena is currently in private testing while we finalize competitive
          systems and verification logic.
        </p>
        <Link
          to="/"
