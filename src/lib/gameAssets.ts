...
export const getGameCategory = (game: string) => {
  ...
  if (normalizedGame === 'Custom' || normalizedGame === 'Other/Custom') {
    return 'Custom';
  }
...
/** When the user picked "Custom" in create flow, `game` is their free-text title — use this so we don't mis-classify as Sports → basketball. */
export type GameImageContext = {
  isCustomGame?: boolean;
};

/** Preset titles from the create-challenge picker (anything else in `game` is treated as legacy custom text). */
const PRESET_ARENA_GAMES = new Set<string>([
  'NBA 2K26',
  'FC 26',
  'Madden 26',
  'EA UFC 6',
...
/**
 * True when this challenge should use `custom.png` for cover art.
 * - Prefer explicit `isCustomGame` from Firestore (new creates).
 * - Legacy: free-text `game` values from the Custom picker had no flag; infer when the name
 *   does not match a preset and `extractGameFromTitle` cannot map it to a known franchise.
 */
export const isChallengeCustomGame = (challenge?: {
  isCustomGame?: boolean;
  rawData?: { isCustomGame?: boolean };
  game?: string;
} | null): boolean => {
  const fromDoc = challenge?.rawData?.isCustomGame ?? challenge?.isCustomGame;
  if (fromDoc === true) return true;
  if (fromDoc === false) return false;

  const game = (challenge?.game || '').trim();
  if (!game) return false;
  if (PRESET_ARENA_GAMES.has(game)) return false;

  return extractGameFromTitle(game) === 'Gaming';
};

export const getGameImage = (game: string, ctx?: GameImageContext) => {
  if (!game || game === 'Gaming') {
    return '/assets/categories/basketball.png';
  }

  if (ctx?.isCustomGame) {
    return '/assets/categories/custom.png';
  }

  const lowerGame = game.toLowerCase().trim();
  if (lowerGame === 'custom' || lowerGame === 'other/custom') {
    return '/assets/categories/custom.png';
  }
