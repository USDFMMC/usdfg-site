...
export function getTrophyProgress(gamesPlayed: number, trophy: Trophy): number {
  ...
  let previousTrophy: Trophy | undefined;
  for (let i = USDFG_RELICS.length - 1; i >= 0; i--) {
    const t = USDFG_RELICS[i];
    if (t.requiredGames < trophy.requiredGames) {
      previousTrophy = t;
      break;
    }
  }
...
export async function checkSpecialTrophy(trophy: Trophy): Promise<boolean> {
  ...
  if (trophy.specialCondition.type === 'totalUsers') {
    ...
    const cap = trophy.specialCondition.maxUsers;
    return cap !== undefined && totalUsers < cap;
  }
