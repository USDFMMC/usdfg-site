...
export function launchPhantomDeepLink(): void {
  ...
  // Prevent duplicate launches, but recover if a previous attempt left the flag stuck (no redirect).
  if (isPhantomConnecting()) {
    const tsRaw = sessionStorage.getItem('phantom_connect_timestamp');
    const ts = tsRaw ? parseInt(tsRaw, 10) : 0;
    const stale = !ts || Date.now() - ts > 120_000;
    if (stale) {
      sessionStorage.removeItem('phantom_connecting');
      sessionStorage.removeItem('phantom_connect_timestamp');
      console.warn('⚠️ Cleared stale phantom_connecting — retrying deep link');
    } else {
  ...
  try {
    // Mark that we're connecting (prevents duplicate clicks); timestamp used for stale recovery.
