...
export default function TrustReviewModal({
  ...
  const StarRating = ({ value, onChange, label }: { value: number; onChange: (v: number) => void; label: string }) => (
    <div>
      ...
      <div className="flex items-center gap-1 mb-1">
        ...
        <span className="ml-2 text-purple-200 font-semibold text-sm">{value}/10</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-xl rounded-xl border border-white/10 bg-[#07080C]/98 p-3 ring-1 ring-purple-500/10 shadow-[0_16px_48px_rgba(0,0,0,0.45)] max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
                ...
                <button
                  ...
                  className={`px-2 py-1 rounded-md text-xs transition-all ${
                    selectedTags.includes(tag)
                      ? 'bg-gradient-to-r from-purple-600 to-orange-600 text-white border border-white/15'
                      : 'bg-zinc-800/60 text-gray-300 border border-zinc-700/50 hover:border-purple-500/35'
                  }`}
                >
                  {tag}
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-zinc-800/60 border border-white/10 rounded-lg text-sm text-white placeholder-gray-500 focus:border-purple-500/45 focus:ring-1 focus:ring-purple-500/25 focus:outline-none resize-none"
              rows={2}
          ...
          <div className="bg-white/[0.04] border border-white/10 rounded-lg p-2 ring-1 ring-purple-500/10">
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              ...
              className="flex-1 px-3 py-2 bg-gradient-to-r from-purple-500 to-orange-500 text-white rounded-lg font-semibold hover:brightness-110 transition-all text-sm border border-white/10 shadow-[0_0_10px_rgba(124,58,237,0.22)]"
            >
              Submit Review
            </button>
          </div>
