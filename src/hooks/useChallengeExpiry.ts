      ...
      for (const challenge of challenges) {
        ...
        if (isExpired && (challenge.status === "active" || challenge.status === "pending")) {
          ...
          try {
            ...
            // Client delete disabled — keep expired status only (no archive/delete).
          } catch (error) {
        ...
        // Client cleanupExpiredChallenge disabled (allow delete: if false on challenges).
        // Expired rows may remain until server-side cleanup is implemented.

        // Completed auto-cleanup off until server job exists.
      }
    };
