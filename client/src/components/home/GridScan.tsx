...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        // Dynamically import face-api.js only if webcam is enabled
  ...
  return (
    <div ref={containerRef} className={`absolute inset-0 w-full h-full pointer-events-none ${className ?? ''}`} style={style}>
      {showPreview && (
